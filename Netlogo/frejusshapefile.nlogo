...
globals [
  ...
  airports-dataset
  ports-dataset
  patch-data ;; data from file

  agglo-frejus-dataset
  voies-frejus-dataset
  pedo-pays-dataset
...
patches-own [ country-name route pedo]
...
to setup
  ...
  set voies-frejus-dataset gis:load-dataset "shapefile/raster-VoiesFrejus/VoiesFrejus_Lbt93.shp"
...
to display-pedo
  gis:apply-raster pedo2-dataset pedo
  let min-pedo gis:minimum-of pedo2-dataset
  let max-pedo gis:maximum-of pedo2-dataset
...
to display-voies-frejus
  ...
  ;gis:set-drawing-color black
  ;gis:draw voies-frejus-dataset 1
...
to show-patch-data
  ...
  [foreach patch-data [three-tuple -> ask patch first three-tuple item 1 three-tuple [set pcolor item 2 three-tuple ]]]
    [user-message "NOT A LIST"]
    display
end
@#$#@#$#@
GRAPHICS-WINDOW
